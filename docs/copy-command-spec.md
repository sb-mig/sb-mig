...
- Dry-run and referenced-asset planning logs progress while scanning stories for references, then prints a summary of referenced assets, asset folders, asset reference occurrences, and story reference occurrences. Dry-run target conflict checks also log progress while planned target paths are checked.
...
- `collapse-draft` publishes published source stories from their source draft/current JSON.
- `save-only` never publishes copied stories and rejects `--publicationLanguages`.
