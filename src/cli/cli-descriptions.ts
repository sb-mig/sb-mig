...
export const copyDescription = `
    ...
    GOTCHAS
        ...
        copy space settings copies use_translated_stories, show_stories_alternative_versions, hide_flag_icons, flag_icons_display_mode, domain, the preview URLs and encode_preview_urls, and writes only the fields that differ. copy space never turns off use_translated_stories or show_stories_alternative_versions when the target already has them on. Preview URLs are merged by name, the way languages are: a source preview URL replaces the target's one of the same name, and preview URLs that exist only in the target are kept. The domain and preview URLs are written exactly as they are, but printed and reported by their origin (scheme and host) only; nothing after the host is shown on screen or in the --outputPath report, and a rejected settings write is reported by its status and field names only.
