...
export const copyDescription = `
    ...
    GOTCHAS
        ...
        copy space settings copies use_translated_stories, show_stories_alternative_versions, hide_flag_icons, flag_icons_display_mode, domain, the preview URLs and encode_preview_urls, and writes only the fields that differ. copy space never turns off use_translated_stories or show_stories_alternative_versions when the target already has them on. Preview URLs are copied exactly, but printed and reported with query-string values redacted, so a preview secret never appears on screen or in the --outputPath report.
