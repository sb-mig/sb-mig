...
export default class Sync extends Command {
  ...
  async run() {
    ...
    if (args.type === 'components' && flags.all && !flags.ext) {
      ...
      syncAllComponents(Boolean(flags.ext), Boolean(flags.presets))
    }
    ...
    if (args.type === 'components' && !flags.all && flags.ext) {
      ...
      if (components.length === 0) {
        ...
        syncProvidedComponents({components, presets: Boolean(flags.presets), packageName: flags.packageName})
      }
    }
    ...
    if (args.type === 'components' && !flags.all && !flags.ext && !flags.lock) {
      ...
      if (components.length === 0) {
        ...
        syncComponents(components, Boolean(flags.ext), Boolean(flags.presets), flags.packageName)
      }
    }
