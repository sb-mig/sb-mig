...
export default class Hello extends Command {
    ...
    static flags = {
      ...
      add: flags.boolean({
        char: 'a',
        description: 'List of components to add to project',
      }),
      copy: flags.boolean({
        char: 'c',
        description:
      ...
      nospace: flags.boolean({
        char: 'n',
        description:
                'Do not create a space for project. (for example when you have one already)',
      }),
    };
    ...
    async run() {
      ...
      if (flags.add && !flags.copy) {
        const spinner = ora('Installing provided components...\n').start()
      ...
      if (flags.add && flags.copy) {
        const spinner = ora('Installing provided components...\n').start()
        const installedComponents = await installProvidedComponents(
          components
        )
        spinner.stop()
